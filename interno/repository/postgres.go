...
func (s *PostgresStore) SeedDemoData(ctx context.Context) error {
	...
	var primeiroAluno model.Aluno
	for i, input := range alunos {
		aluno, err := s.AdicionarAluno(ctx, input)
		if err != nil {
			return err
		}
		if i == 0 {
	...
	if _, _, err := s.CriarFoto(ctx, primeiroAluno.ID, model.FotoInput{URL: primeiroAluno.Foto, Legenda: "Foto de perfil"}); err != nil {
		return err
	}
